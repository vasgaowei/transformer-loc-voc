...
if __name__ == "__main__":
    ...
    from config.default import cfg_from_file
    from config.default import config as cfg
    config_file = '../../configs/ImageNet/imagenet_vgg16_ipc.yaml'

    cfg_from_file(config_file)
    pprint.pprint(cfg)

    cfg.BASIC.ROOT_DIR = os.path.join(os.path.dirname(__file__), '..', '..')
    dataset_dir = os.path.join(cfg.BASIC.ROOT_DIR, cfg.DATA.DATADIR)
    # train_loader = torch.utils.data.DataLoader(
    #     ImageNetDataset(root=dataset_dir, cfg=cfg, is_train=True),
    #     batch_size=cfg.TRAIN.BATCH_SIZE, shuffle=True, num_workers=cfg.BASIC.NUM_WORKERS, pin_memory=True)
    # for images, labels in train_loader:
    #     print(images.shape, labels.shape)

    val_loader = torch.utils.data.DataLoader(
        ImageNetDataset(root=dataset_dir, cfg=cfg, is_train=False),
        batch_size=cfg.TEST.BATCH_SIZE, shuffle=False, num_workers=cfg.BASIC.NUM_WORKERS, pin_memory=True)
    for images, labels, bboxes, names in val_loader:
        print(images.shape, labels.shape, bboxes.shape, names.shape)
