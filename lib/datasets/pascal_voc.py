...
from core.dis_eval import dis_eval
...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self._image_label[idx]
        gt_box = self._image_gt_box[idx]
        image = Image.open(os.path.join(self._image_dir[data_path_i], name+self._image_ext)).convert('RGB')

        image_size = np.array(list(image.size))
        ...
        if self.is_train:
            ...
            gt_box = gt_box.reshape(-1)
            gt_box = " ".join(list(map(str, gt_box)))
            return image, label, gt_box, image_size, name + self._image_ext, data_path_i
